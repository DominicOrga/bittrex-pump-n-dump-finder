...
def check_market(market):
	...
	webbrowser.open("https://bittrex.com/Market/Index?MarketName={}".format(market), new = 0)
...
def update_candidate_markets(listbox, date_label, time_label):
	...
	if result["success"]:
		for market in result["result"]:
			...
			change_24h = (market["Summary"]["Last"] - market["Summary"]["PrevDay"]) / market["Summary"]["PrevDay"]

			if change_24h >= 0.1:
				listbox.insert(END, ("{}%  {}").format(round(change_24h * 100, 1), market["Summary"]["MarketName"]))
...
def init_gui():
	...
	check_button = Button(root, text = "Check", bg = "red", fg = "white", width = 10, command =  lambda: check_market(listbox.get(ACTIVE)))
	check_button.grid(row = 0, column = 0, padx = (15, 15), pady = (50, 0))

	refresh_button = Button(root, text = "Refresh", bg = "blue", fg = "white", width = 10, command = lambda: update_candidate_markets(listbox, date_label, time_label))
